"""
http://www.ecohnoch.cn/2018/12/17/shuxue79/
...
from src.eval import verification
import sklearn
...
# MXNet框架如何自定义evaluation metric --- https://blog.csdn.net/u014380165/article/details/78311231
class AccMetric(mx.metric.EvalMetric):# 在定义类名称的时候，括号里面表示继承哪个类
    def __init__(self):
        self.axis = 1
    ...
    def update(self, labels, preds):
        ...
        # 先用asnumpy()方法将NDArray转换成numpy.ndarray，然后把数值转成32位int型，原来是浮点型，在铺平
...
def parse_args():
    ...
    parser.add_argument('--network', default='r50', help='specify network')# 默认ResNet50
...
def get_symbol(args, arg_params, aux_params):
    ...
    elif args.loss_type == 2: # 计算CosFace Loss
        s = args.margin_s
        m = args.margin_m
        assert (s > 0.0)
        assert (m > 0.0)
...
def train_net(args):
    ctx = []
    # 如果没有，自己export一个就行了 export CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6
    ...
    som = 20
    # 这个som就是你每隔som个batch_size显示一次训练结果（比如损失，准确率等等）
