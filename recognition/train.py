...
from recognition.sample_config import config, default, generate_config
from src.symbols import spherenet

sys.path.append(os.path.join(os.path.dirname(__file__), 'eval'))
from recognition.eval import verification

sys.path.append(os.path.join(os.path.dirname(__file__), 'symbol'))
from recognition.symbol import fresnet
from recognition.symbol import fmobilefacenet
from recognition.symbol import fmobilenet
from recognition.symbol import fmnasnet
from recognition.symbol import fdensenet
...
def parse_args():
    ...
    args, rest = parser.parse_known_args() # 与parse_args()的区别是parse_known_args在接受到多余的命令行参数时不报错,args与与parse_args一样，rest为保存着余下的命令行字符的list
